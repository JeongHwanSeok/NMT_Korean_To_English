...
def cal_teacher_forcing_ratio(learning_method, total_step):
    if learning_method == 'Teacher_Forcing':
        ...
        import numpy as np
        teacher_forcing_ratios = [1.0 for _ in range(int(total_step / 2))]  # 앞의 절반은 teacher forcing
        b = np.linspace(0.0, 1.0, num=int(total_step / 2))[::-1]  # 뒤의 절반은 Scheduled_Sampling
