...
class Trainer(object):  # Train
    ...
    def train(self):
        ...
        val_ratios = cal_teacher_forcing_ratio('Teacher_Forcing', int(total_step / 100)+1)

        step = 0
        attention = None

        for epoch in range(self.epochs):           # 매 epoch 마다
    ...
    def loss_accuracy(self, out, tar):
        ...
        tar_get = tar.view(-1).to(device)
        # out => [batch_size * sequence_len, vocab_size]
        # tar => [batch_size * sequence_len]
        loss = self.criterion(out, tar_get)     # calculate loss with CrossEntropy
        ppl = math.exp(loss.item())         # perplexity = exponential(loss)

        indices = out.max(-1)[1]         # 배열의 최대 값이 들어 있는 index 리턴
        invalid_targets = tar_get.eq(self.en_voc['<pad>'])  # tar 에 있는 index 중 pad index가 있으면 True, 없으면 False
        equal = indices.eq(tar_get)                         # target이랑 indices 비교
        total = 0
