...
from data_helper import create_or_get_voca, LSTMSeq2SeqDataset, LSTMTestDataset
from Transformer.model import Encoder, Decoder, Transformer, greedy_decoder, Beam, Greedy
...
class Trainer(object):  # Train
    ...
    def train(self):
        ...
        #                              weight_decay=0.0001))
        optimizer = InverseSqrt(self.args.encoder_vocab_size, warmup_end_lr=self.args.learning_rate,
...
class AbstractTranslation(object, metaclass=ABCMeta):
    def __init__(self, checkpoint, dictionary_path, x_test_path=None, batch_size=1, beam_search=False, k=3,
                 y_test_path=None, file_name=None):
    ...
    @abstractmethod
    def get_test_loader(self): pass


class Evaluation(AbstractTranslation, ABC):
    def __init__(self, checkpoint, dictionary_path, x_test_path=None, batch_size=1, beam_search=False, k=3,
                 y_test_path=None, file_name=None):
        super().__init__(checkpoint, dictionary_path, x_test_path=None, batch_size=1, beam_search=False, k=3,
                         y_test_path=None, file_name=None)
        self.k = k
        self.beam_search = beam_search
        self.batch_size = batch_size
    ...
    def test(self, model):
        ...
        for i, data in enumerate(self.test_loader):
            src_input, tar_input, tar_output = data
            enc_input = src_input[0].unsqueeze(0)
            if self.beam_search:
    ...
    def tensor2sentence_di(self, indices: torch.Tensor) -> list:
        return super().tensor2sentence_di(indices)

    def tensor2sentence_ko(self, indices: torch.Tensor) -> list:
        return super().tensor2sentence_ko(indices)


class Translation(AbstractTranslation):  # Usage
    def __init__(self, checkpoint, dictionary_path, x_test_path=None, batch_size=1, beam_search=False, k=3,
                 file_name=None):
        super().__init__(checkpoint, dictionary_path, x_test_path=None, batch_size=1, beam_search=False, k=3,
                         file_name=None)
    ...
    def korean2dialect(self, model, sentence: str) -> (str, torch.Tensor):
        ...
        output_sentence = self.tensor2sentence_di(indices)
        print("Korean: ", self.tensor2sentence_ko(a)[0])  # input 출력
        print("Predicted : ", output_sentence[0])  # output 출력
        return output_sentence

    def batch_korean2dialect(self, model):
        if self.x_test_path is None:
            print("x_test_path를 입력해주세요")
            raise TypeError
        elif self.file_name is None:
            print("file_name을 입력해주세요")
            raise TypeError
        test_dataset = LSTMTestDataset(self.x_test_path, self.ko_voc, self.seq_len)
        # dataset을 인자로 받아 data를 봅아냄
        test_loader = DataLoader(test_dataset, batch_size=self.batch_size)
        predicted = []
        try:
            for i, data in enumerate(test_loader):
                ...
                indices = output.view(-1, output.size(-1)).max(-1)[1].tolist()
                a = src_input[0].tolist()
                basic_sentence = self.tensor2sentence_ko(a)[0]
                output_sentence = self.tensor2sentence_di(indices)[0]
                predicted.append(output_sentence)
                print("-------test-------")
                print("Korean: ", basic_sentence)  # input 출력
                print("Predicted : ", output_sentence)  # output 출력
                if i % 1000 == 0:
                    print("--------------complete " + str(i) + "step --------------")
            f = open(self.x_test_path, 'r', encoding='UTF8')
            rdr = f.readlines()
            count = 0
            for line in rdr:
                self.lines.append([line[:-1], predicted[count]])
                count += 1
            self.save_csv()
        except KeyboardInterrupt:
            self.save_csv()
    ...
    def save_csv(self):
        f = open(self.file_name, 'w', newline='', encoding='UTF8')
        wr = csv.writer(f)
        wr.writerows(self.lines)
        f.close()
    ...
    def get_test_loader(self):
        test_dataset = LSTMTestDataset(x_path=self.x_test_path,  ko_voc=self.ko_voc, sequence_size=self.seq_len)
        # dataset을 인자로 받아 data를 뽑아냄
        test_loader = DataLoader(test_dataset, batch_size=self.batch_size)
        return test_loader
